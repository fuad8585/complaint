...
<div class="hash">
    <?php
    if (isset($_GET['h_result'])) {
        $hash_name = $_GET['h_result'];
        echo "<h1 class='hash__name'>#$hash_name</h1>";
    }
    ...
    $hashtag = '#'.$_GET['h_result'];
    $hash_q = "select id from hashtag where hash='$hashtag'";
    $hash_r = mysqli_query($con,$hash_q);
    $hash_f = mysqli_fetch_array($hash_r);
    $hash_id = $hash_f['id'];

    $obj_hash_q = "select object_id from object_hashtag where hashtag_id='$hash_id'";
    $obj_hash_r = mysqli_query($con,$obj_hash_q);
    $obj_hash_f = mysqli_fetch_array($obj_hash_r);
    $obj_hash_id = $obj_hash_f['object_id'];

    $start_from = ($page - 1) * $per_page;

    $get_posts = "select * from object where id='$obj_hash_id' ORDER BY `object`.`rate` DESC LIMIT $start_from, $per_page";
    ...
    <input id='$post_id' type='hidden' name='$post_id' value='' />
        ...
        for ($i = 0; $i < count($hash_arr); $i++) {
            $hash = $hash_arr[$i];
            $hash1 = substr($hash, 1);
            echo "<a href='hashs.php?h_result=$hash1' class='hashs'>$hash</a>";
        }
        echo "</div>
